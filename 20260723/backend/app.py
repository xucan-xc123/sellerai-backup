...
# ── App & CORS ──────────────────────────────────────────────────────────────
app = Flask(__name__)
CORS(app, origins=list(ALLOWED_ORIGINS) if ALLOWED_ORIGINS else "*",
...
if not BACKEND_API_KEY:
    logger.warning("BACKEND_API_KEY is not set — auth is DISABLED! Set it in .env for production.")

ALLOWED_ORIGINS_RAW = os.environ.get("ALLOWED_ORIGINS", "")
ALLOWED_ORIGINS = set(o.strip() for o in ALLOWED_ORIGINS_RAW.split(",") if o.strip()
) if ALLOWED_ORIGINS_RAW else {"http://localhost:3000", "https://sellerai.listaikit.com"}
